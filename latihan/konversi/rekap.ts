...
const rekapAbsensi: DataAbsensi[] = [
...
function durasiKerja(record: DataAbsensi): number{
...
function terlambat(record : DataAbsensi) {
  if (record.status !== "hadir") {
    return false;
  }
...
function buatRekap(data: DataAbsensi[]) {
  let totalHadir = 0;
  let totalJam = 0;
  const namaTerlambat: string[] = [];
...
const hasil = buatRekap(rekapAbsensi);
console.log(hasil);
